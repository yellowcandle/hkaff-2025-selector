...
function AppHKAFFContent() {
  ...
  // INTEGRATION: Load real HKAFF data instead of sample data
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load real HKAFF data from existing data files
        const baseData = await filmService.loadAllData();

        setFilms(baseData.films);
        setScreenings(baseData.screenings.map(convertServiceScreening));
        setVenues(baseData.venues);
        setCategories(baseData.categories.map(convertServiceCategory));
        ...
        console.error('Failed to load HKAFF data:', err);
  ...
  // Convert films to Figma format for FilmBrowser
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <header className="bg-white/90 backdrop-blur-md shadow-sm sticky top-0 z-40 border-b border-gray-200/50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <nav role="navigation" aria-label={isZh ? '主要導航' : 'Main navigation'} className="flex-1 order-1">
                <div className="flex justify-center gap-2 sm:gap-3 max-w-lg mx-auto" role="tablist">
                <button
                  ...
                  className={`flex-1 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full text-xs sm:text-sm font-semibold transition-all duration-200 ${
                ...
                <button
                  ...
                  aria-controls="main-content"
                  className={`relative flex-1 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full text-xs sm:text-sm font-semibold transition-all duration-200 ${
        ...
        {/* Main Content */}
        <main id="main-content" role="main" className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-pink-50">
